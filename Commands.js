...
class MessageFlagBuilder {
  checks = [];
  hook = "command";
  critical = false;

  constructor(hook="all") {
    this.hook = hook; // what messages are checked
    this.uid = this.guid();
  }
  guid() {
    var S4 = function() {
...
class CommandHandler extends EventEmitter {
  ...
  constructor(client, prefix="!") {
    ...
    this.commandFlags = [];
    this.hooks = {
      "all": this.generalFlags,
      "command": this.commandFlags
    }
  ...
  messageHandler(msg) {
    ...
    for (let i in this.generalFlags) {
      if (!this.generalFlags[i].validateMessage(msg, this.request("context"))) {
        ...
        if (this.generalFlags[i].critical) return;
      }
    }
  ...
  addFlag(fBuilder) {
    this.hooks[fBuilder.hook].push(fBuilder);
    return this.hooks[fBuilder.hook];
  }
  setHelpCommand(name) {
...
module.exports = { CommandHandler, CommandBuilder, MessageFlagBuilder };
